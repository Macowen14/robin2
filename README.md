...
For a local checkout, build and start Robin with Docker Compose:

```bash
docker compose up -d --build
```

Open `http://localhost:8501`. To stop it, run:

```bash
docker compose down
```

After the first build, use `docker compose up -d` for normal starts. View logs with `docker compose logs -f`.
